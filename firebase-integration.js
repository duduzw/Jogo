...
function joinPregameRoom(roomCode, playerId, gameMode) {
    ...
    roomRef.once('value').then(snapshot => {
        ...
        // Already in the room (e.g. reconnect) - just re-attach listener
        if (players[playerId]) {
            setupPregameRoomListener(roomCode, playerId);
            return;
        }
        ...
        if (Object.keys(players).length >= maxPlayers) {
            mostrarToast("Erro", "Sala cheia", "danger");
            return;
        }
...
function setupPregameRoomListener(roomCode, playerId) {
    ...
    roomRef.on('value', snapshot => {
        ...
        if (roomData.status === 'starting') {
            // Detach the listener now so the later room-cleanup deletion
            // doesn't fire the "room no longer exists" branch above and
            // yank players back to the connection screen mid-creation.
            roomRef.off('value');
        ...
        // Check if all players are ready (must have at least 2 connected —
        // otherwise a lone host would trivially satisfy "every player is ready")
        const players = roomData.players || {};
        const playerIds = Object.keys(players);
        const allReady = playerIds.length >= 2 && playerIds.every(id => players[id].ready === true);
